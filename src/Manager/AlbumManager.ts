...
export class AlbumManager extends BaseManager {
    public async fetch(id: string, requester: unknown): Promise<SearchResult> {
        try {
            ...
            const response = await fetch(`https://amp-api.music.apple.com/v1/catalog/us/albums/${id}`, { headers: { Authorization: `Bearer ${this.resolver.token}` } });
            ...
            if (this.resolver.plugin.options.cacheTrack) this.cache.set(id, { tracks: fileredData.map(x => ({ name: x.attributes.name, uri: x.attributes.url, artist: x.attributes.artistName, duration: x.attributes.durationInMillis * 1000 })), name: data.data?.filter(x => x.type === "albums")[0]!.attributes.name });
