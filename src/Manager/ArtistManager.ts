...
export class ArtistManager extends BaseManager {
    public async fetch(id: string, requester: unknown): Promise<SearchResult> {
        try {
            ...
            if (this.resolver.plugin.options.cacheTrack) this.cache.set(id, { tracks: fileredData.map(x => ({ name: x.attributes.name, uri: x.attributes.url, artist: x.attributes.artistName, duration: x.attributes.durationInMillis * 1000 })), name: data.data?.filter(x => x.type === "artists")[0]!.attributes.name });
